...
-- 
-- For each company that is a prospect, finds all issues/PRs where:
-- - A GitHub user from that company is the author of a PR or issue in the last 30 days, OR
-- - A GitHub user from that company has interacted with a PR or issue in the last 30 days
--   (via comments, reactions, or PR reviews)
...
WITH prospect_companies AS (
  ...
  WHERE hc.lifecyclestage IN ('lead', 'marketingqualifiedlead', 'salesqualifiedlead', 'opportunity')
),
