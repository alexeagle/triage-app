 ...
 * Updates avatar_url and type if they have changed.
 *
...
export async function upsertGitHubUser(user: GitHubUserInput): Promise<void> {
  await query(
    `INSERT INTO github_users (
      github_id, login, avatar_url, type, first_seen, last_seen
    ...
    DO UPDATE SET
      ...
      type = EXCLUDED.type,
      last_seen = NOW()`,
    [user.github_id, user.login, user.avatar_url ?? null, user.type ?? null],
  );
}
